  ...
  // FIX: parse date string as local time instead of UTC to avoid ±1 day
  // timezone errors in PHT (UTC+8). "2025-08-01" parsed via new Date() is
  // UTC midnight which resolves to 08:00 PHT — off by 8h vs local midnight.
...
// Reconcile late fees for every open loan before reading installments
    await Promise.all(
      loanIds.map(id => supabase.rpc('apply_late_fees_for_loan', { p_loan_id: id }))
    )
...
const enriched = allLoans.map(loan => {
      ...
      const totalLateFees = insts.reduce((s, i) => s + Number(i.late_fee ?? 0), 0)
    const outstanding = Math.max(0, Math.round((Number(loan.total_repayable) + totalLateFees - totalPaid) * 100) / 100)
      const paidCount   = insts.filter(i => Number(i.amount_paid) >= Number(i.amount_due) + Number(i.late_fee ?? 0)).length
const nextInstallment = insts.find(i => Math.round((Number(i.amount_due) + Number(i.late_fee ?? 0) - Number(i.amount_paid)) * 100) / 100 >= 0.05) ?? null
      return { loan, insts, totalPaid, outstanding, paidCount, nextInstallment }
    })

    const openLoans      = enriched.filter(e => ['approved', 'disbursed', 'active', 'overdue'].includes(e.loan.status) && e.outstanding > 0);    const totalOutstanding = openLoans.reduce((s, e) => s + e.outstanding, 0)
    ...
    // QoL: count per filter tab so borrowers can see at a glance what's where
    const filterCounts: Record<string, number> = {
      all:       enriched.length,
      open:      openLoans.length,
      pending:   enriched.filter(e => e.loan.status === 'pending').length,
      completed: enriched.filter(e => e.loan.status === 'completed').length,
      closed:    enriched.filter(e => ['rejected', 'defaulted'].includes(e.loan.status)).length,
    }
    ...
    return (
      <>
        <style>{`
          ...
          .btn-primary.sm:hover { box-shadow:3px 3px 0 var(--ink); }
          ...
          /* Loan row — wrapper div + inner link so we can add a Pay button
            without nesting <a> inside <a> (invalid HTML, React warns). */
          .loan-row-wrap {
            display:flex; align-items:center; gap:8px;
            border-bottom:1px dashed var(--line); transition:background 0.15s ease;
          }
          .loan-row-wrap:last-child { border-bottom:none; }
          .loan-row-wrap:hover { background:var(--paper-2); }
          .loan-row {
            display:flex; align-items:center; justify-content:space-between; gap:16px;
            padding:20px 24px; text-decoration:none; flex:1; min-width:0;
          }
          /* QoL: quick pay button inside the row wrapper */
          .quick-pay {
          ...
          .tab-count {
            ...
            min-width:16px; height:16px; border-radius:999px; font-size:10px; padding:0 3px;
            background:rgba(255,255,255,0.15); color:inherit;
          }
          .filter-tab:not(.active) .tab-count { background:var(--paper-2); color:var(--ink-4); }
          ...
          /* QoL: rejection reason snippet */
          .rejection-snippet {
            font-family:'Space Mono',monospace; font-size:10.5px; color:var(--ink-3);
            background:var(--paper-2); border:1px dashed var(--line-md);
            border-radius:3px; padding:6px 10px; margin-top:6px; line-height:1.5;
          }
        `}</style>
          ...
          <main className="max-w-3xl mx-auto px-6 sm:px-10 py-10">
            ...
            {!loansError && allLoans.length === 0 && (
              <div className="ledger-card empty-state">
                <p className="font-display text-xl mb-2" style={{ color: 'var(--ink)', fontWeight: 500 }}>No loans yet</p>
                <p className="text-sm mb-6" style={{ color: 'var(--ink-3)' }}>
                  You haven't applied for a loan. Start your first application below.
                </p>
                {/* FIX: was /apply — correct path is /loans/apply */}
                <Link href="/loans/apply" className="btn-primary">Apply now</Link>
              </div>
            )}
            ...
            {!loansError && allLoans.length > 0 && (
              <>
                {/* Portfolio summary */}
                {/* FIX: col-span-2 sm:col-span-3 replaces inline gridColumn style so the
                    "Next payment" card fills the row on both 2-col (mobile) and 3-col (sm+) grids */}
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
                  <div className="stat-card">
                    <p className="font-mono text-xs uppercase tracking-widest mb-1" style={{ color: 'var(--ink-4)' }}>Total outstanding</p>
                    <p className="font-display text-lg" style={{ color: 'var(--ink)', fontWeight: 500 }}>{peso(totalOutstanding)}</p>
                  </div>

                  <div className="stat-card">
                    <p className="font-mono text-xs uppercase tracking-widest mb-1" style={{ color: 'var(--ink-4)' }}>Open loans</p>
                  ...
                  {/* FIX: removed inline gridColumn style — Tailwind col-span handles both breakpoints */}
                  <div className="stat-card col-span-2 sm:col-span-3">
                    <p className="font-mono text-xs uppercase tracking-widest mb-1" style={{ color: 'var(--ink-4)' }}>Next payment due</p>
 {soonestDue ? (
                      <p className="font-display text-lg" style={{ color: 'var(--ink)', fontWeight: 500 }}>
                        {peso(Math.max(0, Number(soonestDue.amount_due) + Number(soonestDue.late_fee ?? 0) - Number(soonestDue.amount_paid)))}
                        <span className="font-mono text-xs ml-2" style={{ color: 'var(--ink-3)' }}>
                          due {formatDate(soonestDue.due_date)}
                        </span>
                      </p>
                    ) : (
                      <p className="font-display text-lg" style={{ color: 'var(--ink-4)', fontWeight: 500 }}>Nothing scheduled</p>
                    )}
                  </div>
                </div>

                {/* Filter tabs with QoL count badges — skip tabs with 0 items */}
                <div className="filter-tabs mb-6">
                  {FILTERS.map(f => {
                    const count = filterCounts[f.key]
                    if (count === 0 && f.key !== 'all' && activeFilter.key !== f.key) return null
                    return (
                      <Link
                        key={f.key}
                ...
                {visibleLoans.length === 0 ? (
                  <div className="ledger-card empty-state">
                    <p className="text-sm" style={{ color: 'var(--ink-3)' }}>No loans match this filter.</p>
                  </div>
                ) : (
                  <div className="ledger-card overflow-hidden">
                    <div className="punch-line" />
                    {visibleLoans.map(({ loan, insts, outstanding, paidCount, nextInstallment }) => {
// Show 'Paid' stamp if loan is fully settled even before DB status updates
                      const effectiveStatus = (['disbursed', 'active'].includes(loan.status) && outstanding === 0) ? 'completed' : loan.status
                      const meta  = STATUS_META[effectiveStatus] ?? STATUS_META.rejected
                      const isOpen = ['approved', 'disbursed', 'active', 'overdue'].includes(loan.status) && outstanding > 0
                      // QoL: quick pay visible on active payment-eligible statuses only (not approved/pending-release)
                      const canQuickPay = ['active', 'disbursed', 'overdue'].includes(loan.status) && !!nextInstallment && outstanding > 0
                      const dLeft = nextInstallment ? daysUntil(nextInstallment.due_date) : null

                      return (
                        // QoL: wrapper div keeps loan-row as a link while allowing a sibling Pay button
                        // — avoids nested <a> tags which are invalid HTML
                        <div key={loan.id} className="loan-row-wrap">
                          <Link href={`/loans/${loan.id}`} className="loan-row">
                              ...
                              {isOpen && insts.length > 0 && (
                                <div style={{ maxWidth: 220 }}>
                                  ...
                                  <span className="font-mono text-xs" style={{ color: 'var(--ink-4)' }}>
                                    {paidCount}/{insts.length} installments paid
                                  </span>
                                </div>
                              )}

                              {/* QoL: show rejection reason snippet on rejected loans */}
                              {loan.status === 'rejected' && loan.rejection_reason && (
                                <p className="rejection-snippet">
                                  {loan.rejection_reason.length > 120
                                    ? loan.rejection_reason.slice(0, 120) + '…'
                                    : loan.rejection_reason}
                                </p>
                              )}

{/* QoL: show total paid on completed loans */}
                              {effectiveStatus === 'completed' && (
                                <p className="font-mono text-xs mt-1" style={{ color: 'var(--teal-dark)' }}>
                                  ✓ {peso(Number(loan.total_repayable))} fully repaid
                                </p>
                              )}
                            </div>
                            ...
                            <div className="text-right" style={{ flexShrink: 0 }}>
                              {isOpen ? (
                                <>
                                  <p className="font-mono text-xs mb-1" style={{ color: 'var(--ink-4)' }}>Outstanding</p>
                                ...
                                <>
                                  <p className="font-mono text-xs mb-1" style={{ color: 'var(--ink-4)' }}>Total</p>
                          ...
                          {/* QoL: Quick Pay shortcut — saves borrower a click on their most urgent action */}
                          {canQuickPay && (
                            <div className="quick-pay">
                              <Link
                                href={`/loans/${loan.id}/pay`}
                                className={`btn-primary sm ${loan.status === 'overdue' ? '' : 'opacity-80'}`}
                              >
                                Pay →
                              </Link>
                            </div>
